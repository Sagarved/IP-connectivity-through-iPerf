...
def is_ping_successful():
    try:
        ...
        result_google = subprocess.run(["ping", "-n", "1", GOOGLE_IP],
                                ...
                                stderr=subprocess.DEVNULL)
        # print(result)
        return result.returncode == 0 or result_google.returncode == 0
    except Exception:
        return False

def log_failure():
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    message = f"[{timestamp}] Ping failed more than {MAX_FAILED_PINGS} times."
    print(message)
...
def main():
    failed_count = 0
    print(f"Monitoring ping to {TARGET_IP}... (press Ctrl+C to stop)")
    ...
    while True:
        if is_ping_successful():
            ...
            failed_count += 1
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Ping failed ({failed_count})")
